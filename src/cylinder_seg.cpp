  ...
  while (m_left > 5 && m_left > 0.1 * m) {
    ...
    while (k < K) {
      ...
      b = (N.col(id_1).array() * P_proj.col(id_1).array() + N.col(id_2).array() * P_proj.col(id_2).array() +
           N.col(id_3).array() * P_proj.col(id_3).array())
                  .sum() /
              3 -
          (e1.transpose() * e2)(0) / 9;
      r = b / a;
      ...
      if (dist < min_hypothesis_dist) {
        ...
        if (nr_inliers > nr_inliers_accepted) break;
      }
      k++;
    }

    // Checkpoint 2
    if (max_nr_inliers < 6) break;
    ...
    for (int i = 0; i < m; i++) {
      if (I_final(i)) {
        ...
        if (ids_left_mask(i)) ids_left.push_back(i);
      }
    }
...
float CylinderSeg::distance(Eigen::Vector3f& P3, int id) { return ((P2[id] - P1[id]).cross(P3 - P2[id])).norm() / P1P2_norm[id] - radii[id]; }
