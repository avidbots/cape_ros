...
Cape::Cape(int depth_height, int depth_width, int cell_width, int cell_height, bool cylinder_detection, float min_cos_angle_4_merge,
...
void Cape::process(Eigen::MatrixXf& cloud_array, cv::Mat& seg_out,
    ...
    for (int cell_c = 0; cell_c < nr_horizontal_cells; cell_c++) {
      ...
      if (Grid[stacked_cell_id]->planar) {
        cell_diameter = (cloud_array.block(stacked_cell_id * nr_pts_per_cell + nr_pts_per_cell - 1, 0, 1, 3) -
                         cloud_array.block(stacked_cell_id * nr_pts_per_cell, 0, 1, 3))
                            .norm();
  ...
  while (nr_remaining_planar_cells > 0) {
    ...
    // Checkpoint 1
    if (seed_candidates.size() < 5) break;
    ...
    // Checkpoint 2
    if (nr_cells_activated < 4) continue;
  ...
  for (int i = 0; i < nr_planes; i++) plane_merge_labels.push_back(i);
  ...
  for (int r = 0; r < planes_association_matrix.rows(); r++) {
    ...
    for (int c = r + 1; c < planes_association_matrix.cols(); c++) {
      if (planes_association_matrix(r, c)) {
        ...
        double distance =
            pow(plane_segments[r].normal[0] * plane_segments[c].mean[0] + plane_segments[plane_id].normal[1] * plane_segments[c].mean[1] +
    ...
    if (plane_expanded) plane_segments[plane_id].fitPlane();
  }

  /*------------------------------- Refine plane boundaries -------------------------------*/
  // vector<PlaneSeg> plane_segments_joint;
  for (int i = 0; i < nr_planes; i++) {
    if (i != plane_merge_labels[i]) continue;
      ...
      for (int cell_c = 0; cell_c < nr_horizontal_cells; cell_c++) {
        ...
        if (row_ptr[cell_c] > 0) {
          ...
          distances_cell_stacked = cloud_array.block(offset, 0, nr_pts_per_cell, 1).array() * nx +
                                   ...
                                   cloud_array.block(offset, 2, nr_pts_per_cell, 1).array() * nz + d;
          // Assign pixels
          int j = 0;
          float dist;
...
void Cape::RegionGrowing(unsigned short width, unsigned short height, bool* input, bool* output, vector<PlaneSeg*>& Grid,
  ...
  if (input[index] == false || output[index] == true) return;
